...
| T699H | P6 玩法扩展 | DONE | T330, T410, T660, T699G | 0.8 | 新增可扩展的三角形笔势识别，并触发全体怪物减速。 |
| T700 | P7 质量发布 | READY | T540, T660, T681, T699A, T699D, T699H | 2.0 | 补齐纯规则EditMode回归矩阵。 |
...
### T699B · 完善配置工作簿全部Sheet与字段的中文说明。

- **状态：** `DONE`
- **依赖：** T250, T699A
- **估算：** 0.8 人日
- **产出：** 31个Sheet的中文用途说明；30个数据Sheet第3行中文字段名；`FieldDictionary`全部280个业务字段的非占位中文语义说明；同源配置快照与覆盖测试。
- **明确不做：** 不改第4行英文API字段名、FieldDictionary列结构、Schema版本、DTO、玩法数值、ID、枚举、外键、场景、Prefab、Registry或ProjectSettings；不提前实施T700。
- **验收：** 每个Sheet均有中文用途说明；每个字段均可直接看到中文名称，并能在FieldDictionary查到包含用途/单位/边界或引用语义的中文说明；不存在“X表的Y字段”占位文案；导出表数、记录数与ID计数不变，配置门及Unity回归通过。
- **验证：** 工作簿结构/说明覆盖审计；31个Sheet渲染检查；配置导出器与漂移门；配置相关Unity测试；全量EditMode/PlayMode。
- **证据：** `artifacts/evals/T699B/`
- **提交：** `T699B: document workbook fields in Chinese`

### T699C · 将字段和枚举说明完善为可直接理解的业务语义。

- **状态：** `DONE`
- **依赖：** T699B
- **估算：** 0.8 人日
- **产出：** FieldDictionary全部280个字段的自包含业务说明；Enums全部98个枚举值的行为说明；25个枚举字段的逐值含义与当前运行时限制；同源配置快照和质量测试。
- **明确不做：** 不以说明字数作为质量标准；不修改字段形状、玩法数值、ID、枚举成员、外键、Schema、DTO、场景、Prefab、Registry或ProjectSettings；不提前实施T700。
- **验收：** 阅读字段说明即可知道其控制对象、触发时机、单位/边界、枚举取值效果和当前实现限制中的适用内容；不存在仅复述字段名或“X表的Y字段”式占位文案；导出表数、记录数和ID计数不变，配置门及相关Unity回归通过。
- **验证：** 31个Sheet结构与公式审计、工作簿渲染检查、说明覆盖测试、配置确定性/漂移门、ConfigPipeline EditMode/PlayMode。
- **证据：** `artifacts/evals/T699C/`
- **提交：** `T699C: clarify configuration business semantics`

### T699D · 为生产战斗补齐配置驱动的蓄力按住反馈。

- **状态：** `DONE`
- **依赖：** T330, T340, T660, T699C
- **估算：** 0.5 人日
- **产出：** 静止按住进度事件；触点青白环形蓄力反馈；从蓄力环到正常闪电笔迹的无缝切换；生产入口和生命周期回归。
- **明确不做：** 不改“先按住、再划动”的既有蓄力识别规则，不把纯长按抬手改成攻击；不新增Inspector数值、配置表、动画资源、Animator状态、Scene、Prefab或Registry；不提前实施T700。
- **验收：** 合法起笔后即在触点显示蓄力环，并按`StrokeRules.stroke_charged.chargeHoldSec`从空推进到满；环半径读取同规则`hitRadiusRefPx`，颜色/宽度读取当前架势的画笔样式；首个有效移动点出现后环消失并转为正常轨迹，抬手后继续由T330按最小长度与起笔停留判断Charged；取消、抬手和池化复用不残留环。
...
### T699H · 新增可扩展的三角形笔势识别，并触发全体怪物减速。

- **状态：** `DONE`
- **依赖：** T330, T410, T660, T699G
- **估算：** 0.8 人日
- **产出：** `Triangle`笔势类型和纯几何匹配器；`StrokeRules.onMatchSkillId -> Skills -> SkillEffects`配置化触发链；全体活动普通怪与Boss的减速接入；同源工作簿、Schema、生成物和自动化测试。
- **明确不做：** 不提前实现矩形、五角星等其他形状；不新增形状专属MonoBehaviour、Inspector数值、教学步骤、UI按钮或新美术资源；不改变终极`Circle`、蓄力`Charged`、普通伤害、护甲、弱点和投射物规则；不提前实施T700。
- **验收：** 玩家画出的近似闭合三角形满足配置长度、面积、闭合距离、边缘拟合误差和最小角度时识别为`Triangle`；圆形、矩形及未闭合折线不得误识别；命中规则只通过配置技能ID触发`AllEnemies`效果，场上普通怪和Boss均获得`buff_slow_30`；减速期间移动按配置倍率连续变慢，Buff结束后连续恢复，不发生位置跳变；以后新增形状可新增识别器和配置映射，不把具体效果写入几何算法。
- **验证：** ConfigExporter严格导出/漂移门；T699H EditMode 4/4与生产入口PlayMode 1/1；全量EditMode 223/223、PlayMode 62/62；Unity刷新编译与Console Error/Warning检查。
- **证据：** `artifacts/evals/T699H/`
- **提交：** `T699H: add triangle slow gesture`
...
- **状态：** `READY`
- **依赖：** T540, T660, T681, T699A, T699D, T699H
- **估算：** 2.0 人日
