...
| T699 | P6 内容制作 | DONE | T630, T694, T695, T698 | 0.5 | 整理画笔、主角与敌人的手动创建和导入流程用户手册。 |
...
### T699 · 整理画笔、主角与敌人的内容制作使用手册。

- **状态：** `DONE`
- **依赖：** T630, T694, T695, T698
- **估算：** 0.5 人日
- **产出：** `docs/CONTENT_AUTHORING_GUIDE.md`，串联素材准备、权威配置、Unity导入/生成、AssetRegistry绑定、自动化验证和目视验收。
- **明确不做：** 不修改运行时代码、配置数据、场景、Prefab、动画、美术源文件或Unity YAML；不新增与现有工具不一致的第二套流程。
- **验收：** 当前菜单、路径、配置键和验证命令可由工程源文件核对；覆盖画笔、主角、静态/动画敌人及敌人死亡特效；说明手动创建边界、命名规则、常见故障和交付检查表。
- **验证：** 文档链接与工程路径存在性检查；菜单、配置字段和命令源代码交叉核对；Markdown差异审计。
- **证据：** `artifacts/evals/T699/`
- **提交：** `T699: document content authoring workflows`
