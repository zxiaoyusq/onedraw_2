...
## D-043 · T699A以池化实体统一生产投射物的移动、碰撞与画笔交互

- 状态：ACCEPTED
- 决定：生产Battle为每次投射物攻击从T440 family池租用真实`ProjectileController`，从攻击者当前位置朝玩家生成，并由单一`ProductionProjectileRuntime`按Gameplay时间推进、碰撞和回收。Sprite继续读取`Projectiles.assetKey -> AssetRegistry`，可见尺寸取配置命中直径；Prefab、Inspector和生产代码不保存第二套弹速、寿命、伤害或半径。
- 决定：敌方弹体与玩家身体碰撞体实际相交后才进入玩家受击链；生成和攻击事件本身不再代表伤害。画笔命中沿T350 `HitRecord`进入T370切断/反弹规则；反弹弹体保持原始敌人归属、把当前归属切到玩家并允许伤害敌人，死亡事实继续交给既有计分、关卡与结算链。敌方清弹只回收当前敌方归属，避免误删已经反弹的玩家弹体。
- 决定：配置保持schema `6`并升级content `0.6.7-sample`，只调整五类投射物速度/寿命到可读且可达的160–210参考像素/秒、8–11秒范围。池释放、寿命结束、切断、碰撞与战斗释放都走同一复位边界，清除Sprite、Collider、规则、归属、参考空间、方向与时钟。
- 理由：不可见计数无法表达空间碰撞，也让任意笔迹都能凭空消弹；直接在攻击事件扣HP又绕开命中半径和玩家无敌帧。复用既有实体、池、轨迹命中和归属规则，使玩家看到的弹道与实际伤害/击落判定保持一致。
- 限制：T699A复用当前Registry中的投射物Sprite，不新增正式弹体动画、拖尾、命中特效或音频，不调整敌人攻击表、关卡、波次和出生点；Web、微信DevTools、真机可读性与低端机性能仍属于后续平台/质量任务。

## D-044 · T699D以显式起笔停留时钟驱动不拥有玩法真相的蓄力环

- 状态：ACCEPTED
- 决定：`StrokeInputCollector`继续只从统一指针端口接收Began/Moved/Ended/Canceled，但由会话每帧用同一`Time.unscaledTimeAsDouble`显式调用`Advance`。只有活动笔迹尚未接受首个最小点距外采样时才发布不可变`StrokeHoldProgressEvent`；抖动不结束停留，有效移动、抬手、取消和Dispose都立即停止发布。
- 决定：生产组合根把停留秒数除以`StrokeRules.stroke_charged.chargeHoldSec`得到0到1进度，并用同规则`hitRadiusRefPx`作为环半径。`StrokeTrailView`复用当前架势画笔样式的主体颜色/宽度和Prefab已有分支Renderer绘制固定32段技术拓扑，不创建运行时对象、不新增Inspector数值，也不修改Animator、Prefab或Registry。
- 决定：蓄力环只表示“起笔停留已达到阈值”，不产生Gesture、HitRecord、伤害或教程完成事实。首个有效移动点隐藏环并继续既有实时轨迹；最终Charged仍由T330在抬手后的完整笔迹上同时检查配置最小长度和起笔停留。因此纯长按后抬手不会被升级为攻击，正确操作仍是“按住蓄满，再划动”。
- 理由：只在首个移动点和抬手时记录停留虽然足以离线分类，却让静止按住阶段没有任何玩家可见状态；把动画事件或View进度当成蓄力成功又会建立第二玩法真相。显式时钟事实与只读视图投影能提供即时反馈，同时保持分类和命中裁决唯一。
- 限制：T699D交付程序化青白进度环，不新增主角专用蓄力动画、音频、震动或新美术；32段是固定渲染拓扑而非玩法数值。多设备触控尺寸、Web/微信着色器和真机反馈仍属于后续平台/质量任务。

## D-045 · T699F以配置绑定的独立粒子Prefab承载蓄力表现

- 状态：ACCEPTED
- 决定：`StrokeTrailStyles`新增必填`chargeVfxAssetKey -> AssetManifest.assetKey`，且目标必须为Prefab；当前样式绑定`vfx_stroke_charge`。Registry只保存同键Unity对象引用，颜色、宽度、半径和排序继续由现有画笔/规则配置在运行时覆盖，Prefab不成为第二数值库。
- 决定：`StrokeChargeVfxView`独占4条环线、8条径向电弧和3组ParticleSystem；`StrokeTrailView`不再借用`vfx_slash`分支Renderer，只转发蓄力位置、0到1进度、样式和规则半径。组合根在轨迹池预热阶段解析并实例化全部样式所需Prefab，移动、取消、抬手与回收统一停止和清空，输入热路径不查资源、不创建对象。
- 理由：蓄力拓扑继续写在轨迹View里会让新增粒子、Shader或其他方案必须修改输入表现代码，也会与正常划线Renderer争用；独立资源键和生命周期组件让策划/美术可通过配置与Registry替换Prefab，同时保持手势、命中和伤害真相不变。
- 限制：当前Prefab复现用户选定的A方案且粒子参数由运行时样式派生，不提供游戏内特效选择UI、音频或震动；新增完全不同的技术方案仍需实现相同视图生命周期合同并完成真实设备性能验证。

## D-046 · T699G普通战斗只保留蓄力特殊笔势，终极独占圆形

- 状态：ACCEPTED
- 决定：底层横、竖、斜、弧、圆与蓄力识别器继续保留，但生产普通战斗只装配`Any/Charged`规则；非蓄力有效笔迹不论几何形状都规范化为`Any`。进入`UltimateDrawing`后改用完整分类器，当前终极继续按配置要求`Circle`。
- 决定：配置保持schema `6`并升级content到`0.6.11-sample`。除石甲龟`Charged`防御外，其余防御笔势与全部敌人攻击打断笔势统一为`Any`；缚妖符改为`Any`，终极与终极教学保留`Circle`，第二关蓄力教学、蓄力反馈、架势、护甲、投射物和弱点参数均保持。
- 理由：当前横、竖、斜、弧的数值与表现差异不足以支持可靠的玩家决策，却会让画对空间目标的有效一笔因形状被惩罚。把普通输入收敛到任意有效划动、蓄力和弱点时机，可保留已有命中深度，同时把圆形的明确学习成本集中到高价值终极技能。
- 限制：通用伤害/防御算法仍支持未来由配置重新引入特定笔势，旧规则ID也保持稳定；若以后恢复方向玩法，必须同时补充可辨识敌人提示、独立效果与专项可用性验证，不能只改一个配置字段。

## D-047 · T699H形状识别与技能效果通过配置边界解耦

- 状态：ACCEPTED
- 决定：`GestureClassifier`只负责输出稳定的`GestureType/RuleId`，`StrokeRules.onMatchSkillId`再把命中规则映射到既有`Skills -> SkillEffects`执行链；三角形识别器不得直接引用Buff、敌人或减速实现。当前普通战斗装配`Any/Charged/Triangle`，终极绘制仍使用完整分类器并保持`Circle`门。
- 决定：`TriangleGestureMatcher`从处理后的同一笔迹选取最远点对和距其最远的第三顶点，依次验证配置中的长度、闭合距离、面积、三角形最小内角与所有采样点到候选三边的最大拟合误差。当前只实现`Triangle`；未来形状应以独立纯规则匹配器接入同一分类裁决，不预建空接口层或形状专属MonoBehaviour。
- 决定：减速通过`skill_triangle_slow -> fx_triangle_slow -> AllEnemies -> buff_slow_30`配置为全部活动普通怪和Boss减速30%持续2秒。敌人移动使用累计缩放增量时钟，只缩放Buff生效后的时间段；技能服务继续使用玩法时钟维护冷却，生产敌人目标适配器在效果入口转换为角色状态机的单调时钟。
- 理由：几何形状和玩法效果会分别扩展；稳定的配置映射允许同一种形状替换效果、多个形状复用技能链，也避免形状算法随Buff系统变化。累计移动时钟可防止倍率切换时用总关卡时间重算而产生瞬移。
- 限制：当前没有三角形教学、屏幕提示、专属VFX或其他新形状；阈值仅以当前参考分辨率与自动化样本验证，后续每新增一种形状都必须补正例、手绘容差、相邻形状反例及生产效果回归。
