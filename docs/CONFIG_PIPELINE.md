...
9. T250从同一个`PreparedExport`生成JSON、64位hash+LF旁车，以及位于Config asmdef内的27组/当前308项`ConfigIds.g.cs`；禁止分别实现另一套排序、解析或数值真相。
...
4. 一键脚本会在临时目录再次生成并执行`cmp`；漂移时输出受管文件与预期文件的unified diff，验证通过后运行.NET 56项及Unity分类测试。
5. Unity T250测试断言hash旁车、生成元数据、JSON Runtime hash一致，C#实际编入`OneStrokeDemon.Config.dll`，27组308常量均为稳定ID，并用代表性常量完成类型化配置/Registry查询。
...
3. 兼容合同固定为schema `4`和content `0.4.x`；根版本必须与Global对应行一致，`contentHash`必须与导出器相同的规范化SHA-256算法吻合。
