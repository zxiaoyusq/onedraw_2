...
namespace OneStrokeDemon.Tests.PlayMode.T240
{
    ...
    public sealed class AssetRegistryBootstrapPlayModeTests
    {
        ...
        public IEnumerator BootstrapValidatesRegistryBeforeEnteringMainMenu()
        {
            LogAssert.Expect(
                LogType.Log,
                new Regex("CONFIG_RUNTIME_READY.*hash=e0b0dce.*tables=30.*records=772"));
            LogAssert.Expect(
                LogType.Log,
                new Regex("ASSET_REGISTRY_READY.*configHash=e0b0dce.*entries=78.*prefabs=44.*sprites=16.*audioClips=17.*scenes=1"));
            ...
            Assert.That(AssetRegistryRuntime.Current.Count, Is.EqualTo(78));
