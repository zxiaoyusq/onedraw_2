...
namespace OneStrokeDemon.Tests.PlayMode.T240
{
    ...
    public sealed class AssetRegistryBootstrapPlayModeTests
    {
        ...
        public IEnumerator BootstrapValidatesRegistryBeforeEnteringMainMenu()
        {
            LogAssert.Expect(
                LogType.Log,
                new Regex("CONFIG_RUNTIME_READY.*hash=54885fb2.*tables=28.*records=715"));
            LogAssert.Expect(
                LogType.Log,
                new Regex("ASSET_REGISTRY_READY.*configHash=54885fb2.*entries=76.*prefabs=40.*sprites=18.*audioClips=17.*scenes=1"));
