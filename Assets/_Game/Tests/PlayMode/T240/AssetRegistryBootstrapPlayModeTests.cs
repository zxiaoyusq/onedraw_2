...
namespace OneStrokeDemon.Tests.PlayMode.T240
{
    ...
    public sealed class AssetRegistryBootstrapPlayModeTests
    {
        ...
        public IEnumerator BootstrapValidatesRegistryBeforeEnteringMainMenu()
        {
            LogAssert.Expect(
                LogType.Log,
                new Regex("CONFIG_RUNTIME_READY.*hash=e0dabca.*tables=30.*records=763"));
            LogAssert.Expect(
                LogType.Log,
                new Regex("ASSET_REGISTRY_READY.*configHash=e0dabca.*entries=77.*prefabs=43.*sprites=16.*audioClips=17.*scenes=1"));
