...
namespace OneStrokeDemon.Tests.PlayMode.T230
{
    ...
    public sealed class RuntimeConfigBootstrapPlayModeTests
    {
        ...
        public IEnumerator BootstrapLoadsAndIndexesGeneratedSnapshotBeforeMainMenu()
        {
            LogAssert.Expect(
                LogType.Log,
                new Regex("CONFIG_RUNTIME_READY.*schema=7.*content=0\\.7\\.0-sample.*tables=30.*records=772"));
            ...
            Assert.That(GameplayConfigRuntime.CurrentSummary.RecordCount, Is.EqualTo(772));
