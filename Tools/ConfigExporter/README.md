...
- xlsx 需按冻结顺序包含 `README` 和 30 张数据表，数据表第 4 行是精确匹配的表头。
...
- `ConfigIds.g.cs`位于`OneStrokeDemon.Config` asmdef作用域，按29组稳定键生成当前381个Ordinal排序常量，并嵌入schema/content/hash；UTF-8无BOM、LF结尾、无时间戳。
...
- 31个Sheet及表头、FieldDictionary覆盖、字段类型/空值、`contentVersion`和Schema合同。
...
当前60项测试覆盖三生成物双生成字节一致、受管文件精确匹配、JSON/hash/C#三类漂移、冻结哈希/样例语义一致、表头漂移、区域设置无关性、CLI非零错误码、自校验失败保护旧输出，以及43类只修改内存副本的坏配置。坏配置断言精确错误码、Sheet、Excel行和字段；正式xlsx在测试中保持只读。
